...
result get_results(RooWorkspace* wspace, RooFitResult* res, float cutoff, float upper) {
    ...
    double sum_sf_low = sum_tf->Integral(10, cutoff);
    double sum_sf_low_err = sum_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*sum_params_sf).GetMatrixArray());
    double sum_sf_high = sum_tf->Integral(cutoff, upper);
    double sum_sf_high_err = sum_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*sum_params_sf).GetMatrixArray());
    ...
    double top_sf_low = top_tf->Integral(10, cutoff)/10;
    double top_sf_low_err = top_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*top_params).GetMatrixArray()) /10.;
    double top_sf_high = top_tf->Integral(cutoff, upper)/10;
    double top_sf_high_err = top_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*top_params).GetMatrixArray()) /10.;
    ...
    double vv_sf_low = vv_tf->Integral(10, cutoff)/10;
    double vv_sf_low_err = vv_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*vv_params).GetMatrixArray()) /10.;
    double vv_sf_high = vv_tf->Integral(cutoff, upper)/10;
    double vv_sf_high_err = vv_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*vv_params).GetMatrixArray()) /10.;
    ...
    double z_sf_low = z_tf->Integral(10, cutoff)/10;
    double z_sf_low_err = z_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*z_params).GetMatrixArray()) /10.;
    double z_sf_high = z_tf->Integral(cutoff, upper)/10;
    double z_sf_high_err = z_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*z_params).GetMatrixArray()) /10.;
    ...
    double fake_sf_low = fake_tf->Integral(10, cutoff)/10;
    double fake_sf_low_err = fake_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*fake_params).GetMatrixArray()) /10.;
    double fake_sf_high = fake_tf->Integral(cutoff, upper)/10;
    double fake_sf_high_err = fake_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*fake_params).GetMatrixArray()) /10.;
    ...
    double sum_of_low = sum_tf->Integral(10, cutoff);
    double sum_of_low_err = sum_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*sum_params_of).GetMatrixArray());
    double sum_of_high = sum_tf->Integral(cutoff, upper);
    double sum_of_high_err = sum_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*sum_params_of).GetMatrixArray());
    ...
    double top_of_low = top_tf->Integral(10, cutoff)/10;
    double top_of_low_err = top_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*top_params).GetMatrixArray()) /10.;
    double top_of_high = top_tf->Integral(cutoff, upper)/10;
    double top_of_high_err = top_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*top_params).GetMatrixArray()) /10.;
    ...
    double vv_of_low = vv_tf->Integral(10, cutoff)/10;
    double vv_of_low_err = vv_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*vv_params).GetMatrixArray()) /10.;
    double vv_of_high = vv_tf->Integral(cutoff, upper)/10;
    double vv_of_high_err = vv_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*vv_params).GetMatrixArray()) /10.;
    ...
    double z_of_low = z_tf->Integral(10, cutoff)/10;
    double z_of_low_err = z_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*z_params).GetMatrixArray()) /10.;
    double z_of_high = z_tf->Integral(cutoff, upper)/10;
    double z_of_high_err = z_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*z_params).GetMatrixArray()) /10.;
    ...
    double fake_of_low = fake_tf->Integral(10, cutoff)/10;
    double fake_of_low_err = fake_tf->IntegralError(10, cutoff, 0, res->reducedCovarianceMatrix(*fake_params).GetMatrixArray()) /10.;
    double fake_of_high = fake_tf->Integral(cutoff, upper)/10;
    double fake_of_high_err = fake_tf->IntegralError(cutoff, upper, 0, res->reducedCovarianceMatrix(*fake_params).GetMatrixArray()) /10.;
...
result fit_toy(RooWorkspace* wspace, int n, const RooArgSet* globals) {
    ...
    result yield = get_results(wspace, res, 300, 300);
    yield.of.generated_sum.val = toy_data->sumEntries("(channelCat==channelCat::of) & (obs_x_of>300)");
    yield.sf.generated_sum.val = toy_data->sumEntries("(channelCat==channelCat::sf) & (obs_x_sf>300)");
